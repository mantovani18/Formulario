...
function generatePDFAndSendWhatsApp(formData) {
    try {
        ...
        const whatsappMessage = `🍝 *PASTIFÍCIO SELMI* - Novo Currículo
        
👤 *Candidato:* ${nomeCandidate}
📅 *Data:* ${new Date().toLocaleDateString('pt-BR')}
📱 *Contato:* ${formData.whatsapp || 'Não informado'}

📋 Currículo em PDF anexo.

_Enviado automaticamente pelo formulário online._`;
        ...
        setTimeout(() => {
            ...
            const phoneNumber = '5543996212570'; // Número do WhatsApp
            const encodedMessage = encodeURIComponent(whatsappMessage);
            const whatsappURL = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
            
            // Mostrar instruções ANTES de abrir o WhatsApp
            showWhatsAppInstructions(fileName, whatsappURL);
        }, 2000);
...
function showWhatsAppInstructions(fileName, whatsappURL) {
...
function sendByEmail() {
    ...
    // Gerar PDF e preparar email
    setTimeout(() => {
        generatePDFAndSendEmail(formData);
        showLoadingState(false);
    }, 1000);
}

// Função para gerar PDF e enviar por email
function generatePDFAndSendEmail(formData) {
    try {
        // Criar novo documento PDF (mesma lógica do WhatsApp)
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF();
        ...
        const fileName = `Curriculo_${(formData.nome_completo || 'Candidato').replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`;
        
        // Criar corpo do email
        const emailSubject = `Currículo - ${formData.nome_completo || 'Novo Candidato'} - Pastifício Selmi`;
        const emailBody = `Prezados,

Segue em anexo o currículo do candidato:

👤 Nome: ${formData.nome_completo || 'Não informado'}
📅 Data de envio: ${new Date().toLocaleDateString('pt-BR')}
📱 WhatsApp: ${formData.whatsapp || 'Não informado'}
${formData.vaga_especifica === 'sim' ? `🎯 Vaga de interesse: ${formData.qual_vaga || 'Não especificada'}` : ''}

O currículo completo está em anexo no formato PDF.

Atenciosamente,
Sistema de Recrutamento Online - Pastifício Selmi`;

        // Abrir cliente de email com anexo
        const mailtoLink = `mailto:rh@pastificioselmi.com.br?subject=${encodeURIComponent(emailSubject)}&body=${encodeURIComponent(emailBody)}`;
        
        // Fazer download do PDF primeiro
        doc.save(fileName);
        
        // Mostrar instruções ANTES de abrir o email
        showEmailInstructions(fileName, mailtoLink);
...
// Função para mostrar instruções do email
function showEmailInstructions(fileName, mailtoLink) {
    const instructionsMsg = document.createElement('div');
    instructionsMsg.className = 'email-instructions';
    instructionsMsg.innerHTML = `
        <div class="instruction-header">
        ...
        <div class="instruction-content">
            ...
            <div class="steps">
                <div class="step">
                    <span class="step-number">1</span>
                    <span class="step-text">Clique no botão <strong>"📧 Abrir Email"</strong> abaixo</span>
                </div>
                <div class="step">
                    <span class="step-number">2</span>
                    <span class="step-text">Seu programa de email abrirá automaticamente</span>
                </div>
                <div class="step">
                    <span class="step-number">3</span>
                    <span class="step-text">Clique em <strong>"Anexar arquivo"</strong> ou <strong>"📎"</strong></span>
                </div>
                <div class="step">
                    <span class="step-number">4</span>
                    <span class="step-text">Selecione <strong>"${fileName}"</strong> da pasta Downloads</span>
                </div>
                <div class="step">
                    <span class="step-number">5</span>
                    <span class="step-text">Verifique o conteúdo e clique em <strong>"Enviar"</strong></span>
                </div>
            </div>
            
            <div class="instruction-footer">
                <p>💡 <strong>Vantagem:</strong> Email é mais profissional e organizado!</p>
                
                <div class="instruction-buttons">
                    <button onclick="openEmailNow('${mailtoLink.replace(/'/g, "\\'")}', '${fileName}')" class="btn-email-again">
                        📧 Abrir Email Agora
                    </button>
                    <button onclick="closeEmailInstructions()" class="btn-close">
                        ❌ Cancelar
                    </button>
                </div>
            </div>
...
// Função para abrir email quando usuário clicar
function openEmailNow(mailtoLink, fileName) {
    window.location.href = mailtoLink;
    
    // Atualizar instruções para mostrar que email foi aberto
    const instructions = document.querySelector('.email-instructions');
    if (instructions) {
        const header = instructions.querySelector('.instruction-header strong');
        header.textContent = 'Email Aberto! Complete o Envio';
        
        const firstStep = instructions.querySelector('.step-text');
        firstStep.innerHTML = '<strong>✅ Email aberto!</strong> Continue com os próximos passos';
        
        // Mudar botão
        const button = instructions.querySelector('.btn-email-again');
        button.textContent = '📧 Abrir Novamente';
        button.onclick = () => window.location.href = mailtoLink;
    }
}

// Função para fechar instruções do email
function closeEmailInstructions() {
    const instructions = document.querySelector('.email-instructions');
    if (instructions) {
        instructions.remove();
    }
